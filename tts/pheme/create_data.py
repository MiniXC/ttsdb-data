...
if __name__ == "__main__":
    ...
    args.outputdir = Path("../../data/pheme").expanduser()
    ...
    # audio_array = client.infer(args.text, voice="POD0000007005_S0000568")
    # sf.write(os.path.join(
    #     args.outputdir, f"{args.voice}.wav"), audio_array, 
    #     args.target_sample_rate
    # )

    Path("../../data/pheme").mkdir(parents=True, exist_ok=True)
    ...
    for wav, txt in tqdm(zip(wavs, txts), total=len(wavs)):
        ...
        output_wav = f"{output_dir}/{speaker_id}-{wav.stem}.wav"
        output_txt = f"{output_dir}/{speaker_id}-{wav.stem}.txt"

        text = txt.read_text()
        audio_array = client.infer(text, voice=speaker_id)
        sf.write(output_wav, audio_array, args.target_sample_rate)
