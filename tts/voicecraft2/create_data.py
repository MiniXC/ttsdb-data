...
def parse_arguments():
    ...
    parser.add_argument("-m", "--model_name", type=str, default="830M_TTSEnhanced", choices=[
...
def synthesise(target_transcript, output_dir, name):
    ...
    # create temporary file of the last 6 seconds of the audio
    # temp_audio = tempfile.NamedTemporaryFile(suffix=".wav")
    # wav, sr = librosa.load(orig_audio, sr=None)
    # sf.write(temp_audio.name, wav[-int(6 * sr):], sr)
    # orig_audio = temp_audio.name
    # print(orig_audio)


    # orig_transcript = whisper.transcribe(orig_audio)["text"]
    # w, sr = librosa.load(orig_audio)
    # args.cut_off_sec = (len(w) / sr)

    orig_transcript = args.original_transcript

    args.target_transcript = target_transcript
    ...
    target_transcript = " ".join(orig_split[:cut_off_word_idx+1]) + " " + args.target_transcript
...
if __name__ == "__main__":
    ...
    for wav, txt in tqdm(zip(wavs, txts), total=len(wavs)):
        ...
        synthesise(text, "../../data/voicecraft2", wav_name)
