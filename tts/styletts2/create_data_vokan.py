...
if __name__ == "__main__":
    ...
    for wav in tqdm(sorted(list(Path("../../data/libritts_test_processed").rglob("*.wav"))), desc="Synthesizing"):
        try:
            speaker_id = wav.name.split("-")[0]
            wav_spk = f"../../data/tmp_speakers/{speaker_id}.wav"
            if speaker_id not in speaker_dict:
                speaker_dict[speaker_id] = compute_style(wav_spk)
            ref_s = speaker_dict[speaker_id]
            ...
            print(e)
            print(f"Failed to synthesize {wav}")
            continue